...
- \pagestyle{fancy}
- \fancyhead{}
- \fancyhead[CE,CO]{Climate change life expectancy}
---
...
# Results
We find that climate change could alter life expectancy by `r round(mean(lt_lancet$DIF_MID),2)` years (-2.5 months (`r round(mean(lt_lancet$DIF_LOW),2)` - `r round(mean(lt_lancet$DIF_HIGH),2)` years)) in the average European country (**Fig. 1**). Although the average European country could see a change of -0.22 years, several countries are likely to experience considerably greater reductions in life expectancy. Luxembourg could see a change of up to -2.2 years and the medium-variant of climate hazards could cost the average Spaniard 0.98 years of life.

The overall reduction in $e_0$ is comparable to mortality due to influenza and pneumonia [@arias2013united] in the United States. **Table 1** reports the ranking of Climate Change compared to the major causes of death (COD) in European countries using age-standardised death rates and the standard European population [@pace2013revision] for the year 2013. In three European countries (Spain, Luxembourg, Italy), Climate Change could become a top three COD. In Spain (high = `r round(standard$ASDR_high[which(standard$COUNTRY == "Spain")],2)`), Luxembourg (high = `r round(standard$ASDR_high[which(standard$COUNTRY == "Luxembourg")],2)`), France (high = `r round(standard$ASDR_high[which(standard$COUNTRY == "France")],2)`), and Italy (high = `r round(standard$ASDR_high[which(standard$COUNTRY == "Italy")],2)`) Climate Change could become the leading cause of death if Climate Change is more extreme than anticipated. Only all circulatory diseases combined and all cancers combined would cause more deaths.
